...
function applySlice(
  ...
  for (let i = 0; i < bands; i++) {
    ...
    // as the whole layer sliding in. With a single band there is nothing to
    // alternate against, so travel is suppressed entirely and the effect
    // degrades to the plain fade the spec requires — otherwise the lone band
    // would slide in from one side.
...
function applyWipe(
  ...
  if (fx.shape === 'iris') {
    ...
    const soft = Math.max(fx.softness, 0.01);
    ...
    if (fx.shape === 'barn') {
      ...
      const reach = Math.max(t * (span / 2 + soft), 0.01);
      ...
      const ramp = Math.min(0.5, soft / (2 * reach));
      ...
      const head = -span / 2 + t * (span + soft);
      const [x0, y0] = at(head - soft);
      const [x1, y1] = at(head);
      const grad = mctx.createLinearGradient(x0, y0, x1, y1);
      grad.addColorStop(0, '#fff');
      grad.addColorStop(1, 'transparent');
